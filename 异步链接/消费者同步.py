...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        requeue=重置队列,
        channel.basic_nack(delivery_tag=0, multiple=False, requeue=True)否定回答，可以批量否定
        channel.basic_ack(delivery_tag=0, multiple=False)  肯定应答，rabbitmq可以把消息丢弃，可以批量确认
        channel.basic_reject(delivery_tag=0, requeue=True)  直接拒绝，不能批量否定
        """
    ...
    def cancelcallback(ch, method, properties, body):
        # 一般是转发给生产者，让生产者重新发送or记录错误
        pass

    # 当消费者突然中断，比如ctrl+c时，即可调用此方法，将channel中的消息在做处理
    channel.add_on_cancel_callback(callback=cancelcallback)
